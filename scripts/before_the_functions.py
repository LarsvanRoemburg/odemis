...
squaring = False  # if true, the mask will be altered to be a square
...
for nnn in np.arange(17, 18, 1, dtype=int):  # range(len(data_paths_after)) OR np.arange(4, 9, 1, dtype=int)
    ...
    if dx_pix > 0:
        img_after[:, :dx_pix] = 0
        img_after_blurred[:, :dx_pix] = 1
    elif dx_pix < 0:
        img_after[:, dx_pix:] = 0
        img_after_blurred[:, dx_pix:] = 1
    if dy_pix > 0:
        img_after[:dy_pix, :] = 0
        img_after_blurred[:dy_pix, :] = 1
    elif dy_pix < 0:
        img_after[dy_pix:, :] = 0
        img_after_blurred[dy_pix:, :] = 1
    ...
    binary_end_result2 = binary_closing(binary_opening(binary_closing(binary_end_result2, iterations=1),
                                                       iterations=4), iterations=4)  # first closing and second opening
    print("something")
