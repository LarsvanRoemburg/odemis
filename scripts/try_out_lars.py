...
manual_thr_out_before = [300, 700, 3000, 300, 300, 300, 300, 200]  # make this percentage of max image intensity
manual_thr_out_after = [300, 700, 3000, 300, 300, 300, 300, 8000]  # make this percentage of max image intensity
...
for nnn in np.arange(0, 8, 1, dtype=int):
    ...
    if z_value_before[nnn] != np.pi:
        img_before = np.array(data_before_milling[channel_before[nnn]][0][0][z_value_before[nnn]],
                              dtype=int)
    else:
        img_before = np.array(data_before_milling[channel_before[nnn]], dtype=int)

    if z_value_after[nnn] != np.pi:
        img_after = np.array(data_after_milling[channel_after[nnn]][0][0][z_value_after[nnn]],
                             dtype=int)
    else:
    ...
    histo, bins = np.histogram(img_after.flatten(), bins=2000)
    histo = np.cumsum(histo)
    histo = histo / histo[-1]
    plc = np.min(np.where(histo > 0.99))
