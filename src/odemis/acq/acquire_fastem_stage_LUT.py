...
def acquire_MegaField(mppc, dataflow, descanner, multibeam, stage, ccd, beamshift, mm, field_images, dwell_time):
    ...
    if not image_received.wait(multibeam.dwellTime.value * mppc.cellCompleteResolution.value[0] * mppc.cellCompleteResolution.value[1] + 1):
    ...
    for row in range(field_images[1]):  # y axis, move stage on y,
        for col in range(field_images[0]):  # x axis, move stage on x
            print("acquire field image with col (x): {} and row (y): {} ".format(col, row))
            ...
            # # TODO move into separate function call
            ...
            if not image_received.wait(dwell_time * mppc.cellCompleteResolution.value[0] * mppc.cellCompleteResolution.value[1] + 30):
                ...
                raise TimeoutError("!!!!!timeout during megafield acquisition!!!!!!!!!!!!!!!!!!!!!")
