...
class OdemisBugreporter(object):
    ...
    def _set_description(self, name, email, subject, message, topic_id: int):
        ...
        # self.create_ticket(api_key, report_description, [self.zip_fn]) DEBUG
...
class BugreporterFrame(wx.Frame):
    ...
    def _on_report_sent(self, future):
        try:
            ...
            wx.CallAfter(self._on_report_sent_successful_real)
            # wx.CallLater(0, self._on_report_sent_successful_real)
    ...
    def _on_report_sent_successful(self):
        ...
        """
        # On Ubuntu 18.04, with wxPython 4.0.1, CallAfter() doesn't mix well
        # with the notifications for some unknown reason: the message almost
        # immediately disappears. This doesn't happen with CallLater(). Note
        # that CallLater() must be called from the main GUI thread.
        wx.CallLater(0, self._on_report_sent_successful_real)

    def _on_report_sent_successful_real(self):
        """
        See _on_report_sent_successful()
        """
        notify2.init("Odemis")
        notif = notify2.Notification("")
        notif.update("Odemis bug-report successfully uploaded",
